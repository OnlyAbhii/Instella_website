...
export default function BackupPage() {
  ...
  const handleDownload = (file: BackupFile) => {
    setDownloading(file.tagName)

    try {
      const a = document.createElement("a")
      a.href = file.downloadUrl
      a.setAttribute("download", `${file.tagName}.json`)
      ...
      setTimeout(() => {
        setDownloading(null)
        setDownloadSuccess(file.tagName)
      ...
      <main className="container mx-auto px-4 pt-24 pb-16">
        <AnimatePresence mode="wait">
          {selectedBackup ? (
              ...
              <Card className="mb-6 border border-white/10 bg-black/50 backdrop-blur-sm">
                <CardContent className="p-6">
                  <div className="mb-6 grid gap-4 md:grid-cols-2">
                    <div><h3 className="mb-1 text-sm font-medium text-gray-400">Version</h3><p>{selectedBackup.tagName}</p></div>
                    <div><h3 className="mb-1 text-sm font-medium text-gray-400">Size</h3><p>{selectedBackup.size}</p></div>
                    <div><h3 className="mb-1 text-sm font-medium text-gray-400">Date</h3><p>{selectedBackup.date}</p></div>
                    <div><h3 className="mb-1 text-sm font-medium text-gray-400">Creator</h3><p>{selectedBackup.creator}</p></div>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="rounded-lg bg-white/5 p-4">
                      <ul className="list-inside list-disc space-y-2 text-sm text-gray-300">
                        {selectedBackup.changes.map((change, i) => <li key={i}>{change}</li>)}
                      </ul>
                    </div>
                  </div>
                  ...
                  <Button
                    ...
                    disabled={downloading === selectedBackup.tagName}
                  ...
                  >
                    ...
                    ) : downloadSuccess === selectedBackup.tagName ? (
                      <span className="flex items-center">
            ...
            <motion.div key="list" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="mx-auto max-w-3xl">
              <motion.h1 className="mb-8 text-center text-3xl font-bold" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                Library Backup Files
              </motion.h1>

              <div className="flex justify-center mb-6">
                <Button onClick={fetchReleases} className="bg-white text-black hover:bg-gray-200">
                  Refresh Backups
                </Button>
              </div>
              ...
              {loading ? (
                <div className="flex flex-col items-center justify-center py-12">
                  <motion.div className="mb-4 h-12 w-12 rounded-full border-4 border-t-white border-r-transparent border-b-white border-l-transparent" animate={{ rotate: 360 }} transition={{ duration: 1.5, repeat: Infinity, ease: "linear" }} />
                ...
                <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-6 text-center">
                  ...
                  <Button className="mt-4 bg-white text-black hover:bg-gray-200" onClick={fetchReleases}>Try Again</Button>
                </div>
              ) : backups.length === 0 ? (
                ...
                <motion.div variants={container} initial="hidden" animate="show">
                  {backups.map((file, index) => (
                    <motion.div key={file.name} variants={item} transition={{ delay: 0.1 * index }}>
                      <Card className="mb-4 overflow-hidden border border-white/10 bg-black/50 backdrop-blur-sm hover:border-white/30 cursor-pointer" onClick={() => setSelectedBackup(file)}>
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              <motion.div whileHover={{ rotate: 15 }} transition={{ duration: 0.2 }} className="mr-3">
                                <FileText className="h-5 w-5 text-white" />
                              </motion.div>
                              <div>
                                <h3 className="text-lg font-medium">{file.tagName}.json</h3>
                                <div className="flex items-center text-sm text-gray-400">
      ...
      <footer className="border-t border-white/10 bg-black py-8">
        <div className="container flex flex-col items-center justify-between space-y-4 px-4 md:flex-row md:space-y-0">
          ...
          <div className="flex space-x-6">
            <Link className="text-sm text-gray-400 hover:text-white" href="/download">Download</Link>
            <Link className="text-sm text-gray-400 hover:text-white" href="/about">About</Link>
