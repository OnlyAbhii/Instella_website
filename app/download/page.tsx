...
const [bitType, setBitType<"64" | "32">("64"
))] = useState<"64" | "32">("64")
const [cloneType, setCloneType<"normal" | "clone">("normal"
))] = useState<"normal" | "clone">("normal")
const [selectedVersion, setSelectedVersion<Version | null>(null
)] = useState<Version | null>(null)
const [step, setStep<"bitSelect" | "cloneSelect" | "versionSelect" | "versionDetail">("bitSelect"
))] = useState<"bitSelect" | "cloneSelect" | "versionSelect" | "versionDetail">("bitSelect")
...
const processReleases = (releases: GitHubRelease[]) => {
  ...
  for (const release of sortedReleases) {
    ...
    const versionMatch = release.name?.match(/v?(\d+\.\d+(\.\d+)*)/) || release.tag_name?.match(/v?(\d+\.\d+(\.\d+)*)/)
                  ...
                  <CardContent className="p-6">
                    ...
                    {loading ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-gray-400">There are currently no versions available for this configuration.</p>
                      </div>
                    ) : (
                                ...
                                <div className="flex items-center justify-between">
                                  <div>
                                    <div className="flex items-center">
                                      ...
                                      {version.isLatest && (
                                        <Badge className="ml-2 bg-white text-black">Latest</Badge>
                                      )}
                                    </div>
